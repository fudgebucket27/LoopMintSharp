...
else if (args[0].Trim() == "-mintredpacketnft")
{
    ...
    var mintRedPacketNft = await minter.MintRedPacketNft(loopringApiKey, loopringPrivateKey, layer1PrivateKey, minterAddress, accountId, nftBalance, validUntil, maxFeeTokenId, exchange, amount, verboseLogging);
}
else
        {
