...
If you want to mint metadata JSONs that are cidv1(they start with 'b' not 'Qm'), you will need to install ipfs desktop locally. Ideally the source files should be hosted on https://nft.storage. Using https://nft.storage is free! The metadata will be pinned locally to your ipfs desktop instance in order to retrieve the equivalent cidv0 which is used to mint with. You do not need to keep your ipfs desktop instance running all the time only when you want to mint.
