...
Each line contains a comma seperated list of values. The first value is the nftData. To collect nftData you can use maize to retrieve it: https://github.com/cobmin/Maize, otherwise if you minted with LoopMintSharp the CSV report from a previous mint will contain the nftData.The second value is the amount of red packets, the third value is the amount of nfts per red packet, the fourth value only applies to blind and is the random amount of nfts to give out per box, leave this as 0 if its not a blind red packet, the fifth value is the amount of days the red packet is valid for and the sixth value is the red packet type, it can be blind,random or normal.
